...
func (t *TodoListPostgresql) GetAll(userId int) ([]todo.ToDoList, error) {
	...
	query := fmt.Sprintf("SELECT tl.id, tl.title, tl.description FROM %s tl INER JOIN %s ul on tl.id = ul.list_id WHERE ul.user_id = $1", todoListsTable, userListsTable)
...
func (t *TodoListPostgresql) GetById(userId, listId int) (todo.ToDoList, error) {
	...
	query := fmt.Sprintf(`SELECT tl.id, tl.title, tl.description FROM %s tl INER JOIN %s ul on tl.id = ul.list_id WHERE ul.user_id = $1 AND ul.list_id = $2`, todoListsTable, userListsTable)
...
func (t *TodoListPostgresql) Update(userId, ListId int, input todo.UpdateListInput) error {
	...
	query := fmt.Sprintf("UPDATE %s tl SET FROM %s ul WHERE tl.id = ul.list_id AND ul.list_id=$%d AND ul.user_id=$%d", todoListsTable, setQuery, userListsTable, argsId, argsId+1)
