...
func (t *TodoItemPostgresql) Create(listId int, item todo.ToDoItem) (int, error) {
	...
	var itemId int
	createQueryItem := fmt.Sprintf("INSERT INTO %s (title , description) VALUES ($1, $2)", todoItemsTable)
	...
	createQueryListItems := fmt.Sprintf("INSERT INTO %s (list_id, item_id) VALUES ($1, $2)", listItemsTable)
...
func (t *TodoItemPostgresql) GetAll(userId, listId int) ([]todo.ToDoItem, error) {
	...
	query := fmt.Sprintf("SELECT ti.id, ti.title, ti.description, ti.done FROM %s ti INNER JOIN %s li on li.item_id = ti.id INNER JOIN %s ul on ul.list_id = li.list_id WHERE li.list_id = $1 AND ul.user_id = $2 ", todoItemsTable, listItemsTable, userListsTable)
...
func (t *TodoItemPostgresql) GetItemId(userId, itemId int) ([]todo.ToDoItem, error) {
	...
	query := fmt.Sprintf("SELECT ti.id, ti.title, ti.description, ti.done FROM %s ti INNER JOIN %s li on li.item_id = ti.id INNER JOIN %s ul on ul.list_id = li.list_id WHERE ti.id = $1 AND ul.user_id = $2 ", todoItemsTable, listItemsTable, userListsTable)
...
func (t *TodoItemPostgresql) Delete(userId, itemId int) error {
	query := fmt.Sprintf("DELETE FROM %s ti USING %s li, %s lu WHERE ti.id = li.item_id = AND li.list_id = ul.list_id AND ul.user_id = $1 AND ti.id = $2 ", todoItemsTable, listItemsTable, userListsTable)
...
func (t *TodoItemPostgresql) Update(userId, itemId int, input todo.UpdateItemListInput) error {
	...
	query := fmt.Sprintf("UPDATE %s ti SET %s FROM %s li, %s ul WHERE ti.id = li.item_id AND li.list_id = ul.list AND ul.user_id = $%d AND ti.id = $%d",
		todoItemsTable, setQuery, listItemsTable, userListsTable, argsId, argsId+1)
